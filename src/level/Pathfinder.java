...
public class Pathfinder {
	...
	public ArrayList<Tile> findPath(Organism org, int x1, int y1, int x2, int y2, boolean diagonal)
	{
		for (int r = 0; r < grid.rows(); r++)
		{
			for (int c = 0; c < grid.cols(); c++)
		...
		do
		{
			...
			ArrayList<Node> ns = findValidNeighbors(current,org,diagonal);
	...
	public ArrayList<Tile> findAdjustedPath(Organism org, int x1, int y1, int x2, int y2)
	{
		ArrayList<Tile> temp = findPath(org,x1,y1,x2,y2,true);
	...
	public ArrayList<Node> findValidNeighbors(Node node, Organism civ, boolean diagonal)
	{
		...
		try
		{
			if (getNode(r+1,c) != null)
			{
				temp.add(getNode(r+1,c));
			}
			if (getNode(r-1,c) != null)
			{
				temp.add(getNode(r-1,c));
			}
			if (getNode(r,c+1) != null)
			{
				temp.add(getNode(r,c+1));
			}
			if (getNode(r,c-1) != null)
			{
				temp.add(getNode(r,c-1));
			...
			if (diagonal)
			{
				if (getNode(r+1,c+1) != null)
				{
					temp.add(getNode(r+1,c+1));
				}
				if (getNode(r+1,c-1) != null)
				{
					temp.add(getNode(r+1,c-1));
		...
		//TODO: Do not let organisms move through each other
		return temp;
	}
